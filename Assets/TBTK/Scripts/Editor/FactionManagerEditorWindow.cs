	...
	public class FactionManagerEditorWindow : EditorWindow {
		...
		Vector2 DrawFactionConfigurator(float startX, float startY, Faction faction){
			...
			if(!faction.isPlayerFaction){
				...
				EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width+50, height), cont);
				faction.useDefaultAIMode=EditorGUI.Toggle(new Rect(startX+spaceX, startY, 15, height), faction.useDefaultAIMode);
				
				if(!faction.useDefaultAIMode){
					...
					aiMode = EditorGUI.Popup(new Rect(startX+spaceX+20, startY, width-20, 15), cont, aiMode, contList);
