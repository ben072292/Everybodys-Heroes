	...
	public class GameControl : MonoBehaviour {
		...
		public static void SelectUnit(Unit unit){
			if(!FactionManager.IsPlayerFaction(unit.factionID)){
				//Debug.Log ("hahaha");
				ClearSelectedUnit();
				Debug.Log("haha" + unit.unitName);
				selectedUnit = unit;
				//AIManager.MoveUnit(unit);
			}
			else{
				//Debug.Log ("hahaha" + unit.unitName);
				ClearSelectedUnit();
				selectedUnit=unit;
				GridManager.Select(unit);
				unit.Select();
				if(unit.unitName == "TOWER" || unit.unitName == "CREEP"){
					Debug.Log("Tower is always AI controlled");
					AIManager.MovePlayerAIUnit (unit);
				}
			}
		}
