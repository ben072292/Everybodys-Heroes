...
namespace TBTK {
	...
	public class UIOverlay : MonoBehaviour {
		...
		IEnumerator TextOverlayRoutine(Text txt){
			...
			float duration=0;
			while(duration<1){
				txtT.localPosition+=new Vector3(0, 1000*Time.deltaTime, 0);
		...
		void Update(){
			if(GameControl.GetGamePhase()==_GamePhase.Play){
				for(int i=0; i<unitOverlayList.Count; i++){
					...
					Vector3 screenPos = mainCam.WorldToScreenPoint(overlay.unit.thisT.position);

					if(mainCam.velocity.magnitude > 0.5f){
						for(int j=0; j<unitOverlayList.Count; j++){
							unitOverlayList[j].rootObj.SetActive(false);						
						}
					}

					overlay.rootT.localPosition=(screenPos+new Vector3(0, -40, 0))/UI.GetScaleFactor();
		...
		public void OnHoverOverlayButton(GameObject overlayObj){
			CameraControl.enableTouchPan = false;
			for(int i=0; i<unitOverlayList.Count; i++){
		...
		public void OnHoverExitOverlayButton(){
			CameraControl.enableTouchPan = true;
