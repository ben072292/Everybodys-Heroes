...
namespace TBTK {
	...
	public class CameraControl : MonoBehaviour {
		...
		public static bool enableTouchPan=true;
		...
		void Update () {
			...
			if (GameControl.selectedUnit == null)
				return;
			else if (GameControl.selectedUnit.unitName == "TOWER") {
				StartCoroutine (MoveCamera (GameControl.selectedUnit.thisT));
				TouchZoom();
				return;
			}
			else if (GameControl.selectedUnit.unitName != "TOWER" && GameControl.selectedUnit.unitName != "CREEP") {
				if(!oneTime){
					StartCoroutine (MoveCamera (GameControl.selectedUnit.thisT));
					oneTime = true;
				}
		...
		IEnumerator MoveCamera(Transform target){
			while (Vector3.Distance(thisT.position, target.position) > 0.05f){
				thisT.position = Vector3.Lerp(thisT.position, target.position, Time.deltaTime);
				yield return null;
			}
		...
		public static void resetOneTime(){oneTime = false;}
