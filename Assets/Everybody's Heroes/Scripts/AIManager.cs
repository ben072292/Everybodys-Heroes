	...
	public class AIManager : MonoBehaviour {
		...
		IEnumerator FactionRoutine(Faction faction){
			...
			for(int i=0; i<faction.allUnitList.Count; i++){
				GameControl.selectedUnit = faction.allUnitList[i];
		...
		public static void MovePlayerAIUnit(Unit unit){
			instance._MovePlayerAIUnit (unit);
		}
		
		public void _MovePlayerAIUnit (Unit unit){
			StartCoroutine(SingleUnitRoutinePlayerAI(unit));
		}
		
		IEnumerator SingleUnitRoutinePlayerAI(Unit unit){
			yield return new WaitForSeconds(0.5f);
		...
		IEnumerator MoveUnitRoutine(Unit unit, _AIMode activeMode){
			...
			if (targetTile.hostileInRangeList.Count == 0 && unit.unitName == "TOWER") {
